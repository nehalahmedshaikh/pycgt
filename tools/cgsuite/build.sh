...
major="$("$javac_cmd" -version 2>&1 | sed -E 's/javac ([0-9]+).*/\1/')"
if [ "$major" != "17" ]; then
...
if [ ! -d "$cgsuite/.git" ]; then
    ...
    git clone --depth 1 --branch "$cgsuite_ref" \
        https://github.com/aaron-siegel/cgsuite.git "$cgsuite"
fi

# --- compile only; `package` runs a PostBuildScript step that fails -------
echo "== compiling cgsuite-core"
mvn -q -f "$cgsuite/lib/core/pom.xml" compile

# --- classpath, from Maven (a dozen transitive jars) ----------------------
echo "== resolving classpath"
mvn -q -f "$cgsuite/lib/core/pom.xml" \
...
"$java_cmd" -cp "$scalac_cp" scala.tools.nsc.Main \
